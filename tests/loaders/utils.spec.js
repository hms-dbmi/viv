...
test('getChannelStats: Small', async t => {
  ...
  try {
    ...
    t.deepEqual(domains, [[0, 3], [0, 2], [0, 1]]);
...
test('URL join suffixes', t => {
  t.equal(joinUrlParts('https://example.com', 'bla'), 'https://example.com/bla');
  t.equal(
    joinUrlParts('https://example.com/my-store', 'arr.zarr'),
    'https://example.com/my-store/arr.zarr'
  );
  t.equal(joinUrlParts('https://example.com/', 'arr.zarr'), 'https://example.com/arr.zarr');
  t.equal(joinUrlParts('https://example.com/', '', 'arr.zarr'), 'https://example.com/arr.zarr');
  t.end();
});
