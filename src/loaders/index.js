...
async function createZarrPyramid({ rootZarrUrl, minZoom, isRgb, scale, dimNames }) {
...
async function createTiffPyramid({ channelNames, channelUrls }) {
  ...
  return new TiffPyramidLoader(resolvedTiffConnections, channelNames);
}


// eslint-disable-next-line consistent-return
export async function initPyramidLoader(type, { sourceChannels, minZoom, isRgb, dimNames, scale }) {
  let loader;
  if (type === 'zarr') {
    const rootZarrUrl = Object.values(sourceChannels)[0];
    loader = await createZarrPyramid({ rootZarrUrl, minZoom, isRgb, dimNames, scale });
  } else if (type === 'tiff') {
    const channelNames = Object.keys(sourceChannels);
    const channelUrls = Object.values(sourceChannels);
    loader = await createTiffPyramid({ channelNames, channelUrls});
  }
  return loader
}