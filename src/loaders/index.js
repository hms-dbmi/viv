...
export async function initPyramidLoader(type, { sourceChannels, minZoom, isRgb, scale, dimNames, pool }) {
  let loader;
  if (type === 'zarr') {
    const rootZarrUrl = Object.values(sourceChannels)[0];
    loader = await createZarrPyramid({ rootZarrUrl, minZoom, isRgb, scale, dimNames });
  } else if (type === 'tiff') {
